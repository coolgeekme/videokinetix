...
- **Tests**: 27 cases via `node --test tests/*.test.mjs` (`captureQuality` 9, `repDetection` 3, `targetTracking` 15).
...
## Bug fixes (Sep 2026 – v1.8 Swimming: analysis silently failed whenever a stroke peak landed on an undetected frame)
- **Symptom**: swim sessions sometimes reported "no strokes detected" with no error, on clips where strokes were clearly visible. Also produced a hard `TypeError: Cannot read properties of null (reading '15')` in `repDetection.js` when the same code was driven directly.
- **Root cause**: `PoseCanvas` records `{t, lm: null}` for every frame in which the target wasn't confirmed, so the timeline stays continuous. A stroke peak can land on one of those frames, and `analyzeSession` then passed that `null` straight into the sport-specific `apex`/`validatePeak` code — which dereferences landmarks (`lm[L.LEFT_WRIST]?.y` guards the property, not `lm`). Fixed by `const detectedPeaks = peaks.filter((p) => frames[p.index]?.lm)` before any sport code runs; a peak with no landmarks is not an analysable rep.
- **Blast radius**: the caller wraps `analyzeSession` in try/catch and falls back to `{ rep_count: 0, no_reps_detected: true }`, so this surfaced as a silently useless report rather than an error — on exactly the sessions swimming produces most, because swimming loses the athlete for a large share of frames (13% of frames with no detection on the clean reference clip; 34% on the user's first clip).
- **Tests**: `frontend/tests/repDetection.test.mjs` — reproduces the throw against the shipped `apex` body, asserts `analyzeSession` survives peaks landing on undetected frames, that every reported rep references a frame that has landmarks, and that a clean capture still finds its ~5 strokes in 6s.

## Observation (Sep 2026): above-water vs below-surface framing, measured
- Two user clips of the same freestyle session, run through the shipped pipeline (`evaluate_clip.mjs` harness):
  | clip | setup | detected frames | guard | tracker |
  |---|---|---|---|---|
  | IMG_0601 | above water, oblique down-the-lane | 23/141 (16%) | **unusable** — confirmed in 12% of frames | 17 confirmed |
  | IMG_0596 | underwater, side-on | 9/160 (6%) | **unusable** — only 9 frames captured | 1 confirmed |
- Frame-by-frame inspection with landmarks drawn back on: in the above-water clip the swimmer is submerged and invisible for ~84% of frames, and 3 of 4 inspected detections were false positives on the lane rope / splash / water surface. In the underwater clip the swimmer was real but spent most of the clip at the left frame edge (partially cropped), which is why so few frames detected.
- Conclusion recorded for product direction: above-water and below-surface are not two views of one analysis, they see different phases of the stroke. Below surface (side-on, mid-depth) is the only plane that contains the catch/pull the current swim metrics measure; above water can support stroke rate/rhythm, entry, head position and body line, but never the pull. Any above-water mode needs its own metric set and its own framing guidance.
